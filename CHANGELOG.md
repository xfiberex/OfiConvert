...
- **Los scripts de PowerShell, vigilados por codificación:** todo `.ps1` del repo tiene que empezar por
  el BOM de UTF-8, y ninguno puede leer el `.csproj` con `Get-Content`. `tools/capture-dropdown.ps1`
  no lo tenía y sacaba los acentos corruptos en PowerShell 5.1; `build-installer.ps1` leía el
  `.csproj` justo de la forma prohibida. *(Cierra [TJ-27](ROADMAP.md) y [TJ-37](ROADMAP.md).)*
- `LoggingService.CreateLogger` construye el logger con carpeta y límite propios, para probar la
  rotación con un archivo real de 1 KB.
- **CI en GitHub Actions** (`windows-latest`): compilación con `-warnaserror` y las dos suites, UI
  incluida, en cada push y PR a `main`.
